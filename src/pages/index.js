...
class IndexPage extends Component {
  ...
  handleSubmit = event => {
    ...
    if (phone_ct!=''){
      ...
      phone_ct= '7' +phone_ct;
      window.ctw.createRequest('exeedrostokino.ru', phone_ct, [], function(success, data){ console.log(success, data); } );
    }
    // React.useEffect(() => {
  ...
  render() {
    ...
    <div>
      <Helmet>
      <meta charSet="utf-8" />
      <title>РОСТОКИНО ПРЕМИУМ | официальный дилер CHERYEXEED в Москве | Купить CHERYEXEED TXL с выгодой до 180 000 руб.</title>
      <meta name="description" content="РОСТОКИНО ПРЕМИУМ — официальный дилер CHERYEXEED в Москве. Купить CHERYEXEED TXL с выгодой до 180 000 руб."/>
      <meta name="keywords" content="cheryexeed, txl, chery exeed, чери эксид, официальный дилер"/>
      ...
      <Complectations data={JSONData.complectations} handleModalOpen={this.handleModalOpen}/>
      ...
      <ReactModal
        ...
        <div className="text-right pr-2"><button onClick={this.handleModalClose} className="text-lg">x</button></div>
        <div className="font-bold text-2xl mb-4 leading-none text-center">Оставьте заявку<br/>и мы вам перезвоним</div>
        <form id="form1" onSubmit={this.handleSubmit}>
          <div className="mb-2">
            <InputMask mask="+7 (999) 999-99-99" ref="phone" onChange={this.onChange} value={this.state.phone} name="phone" placeholder="Ваш телефон" className="shadow text-center appearance-none border border-exeed-orange w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
          </div>
      ...
      <ReactModal
        ...
        className="w-11/12 sm:w-2/3 lg:w-2/5 xl:w-1/4 px-6 py-3 shadow mx-auto bg-bronze text-white">
        <div className="text-right pr-2"><button onClick={this.handleModalClose2} className="text-lg">x</button></div>
        <div className="font-bold text-2xl mb-4 leading-none text-center">Ваша заявка отправлена</div>
      </ReactModal>
    </div>
    )
  }
